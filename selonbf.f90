...
! REAL, PARAMETER :: K0   = 6.180601  !  Capital stock
...
INTEGER, PARAMETER :: MAXAGE = 69        !  Maximum age allowed, 17-85 which is 69 periods
...
INTEGER ITHETA, ITER, ITERINC, JA, bind
...
IF (MODE==1) THEN   ! This is HP linux
	...
	OPEN(UNIT=8,FILE='/home/sean/Desktop/Dropbox/cultural_ss/pssurfurv.txt')
...
777 format (1X,I2,1X,4(F10.6,1X))
...
!2.4 Compute age profiles and average lifetime utility
	 ...
	 DO AGE=1,MAXAGE
		 IF ((DLONG(AGE)>=NGRID).and.(bind==1)) THEN
            PRINT *, 'Error:  Maximum asset limit is binding.'	! if it's off the decision rule, it doesn't matter
            PRINT *, 'AGE =', AGE
! 			print *,'  '
! 			print *,'  '
...
! rearranging the equation, we can have the following:
       ...
       write (17,777) initm, K, KDEV, BEQ, EXDEM   
