...
REAL, PARAMETER :: ALPHA  = 0.5     !  Labor exponent in production function
...
REAL, PARAMETER :: AMAX   = 100.0      !  Maximum permissible asset
...
INTEGER, PARAMETER :: JMAX = 360000	! JMAX = 1.5 * AMAX / CINCR
...
integer,parameter :: binitgrid=100

real kinitspace(kinitgrid),kinitstep
real binitstep,binitspace(binitgrid)
integer initm,initn
...
do initn=1,binitgrid
	BEQ=binitspace(initn)
    PRINT*,'BINIT=',initn
...
!       with BEQ in judging convergence.
     ...
     write (17,777) K, K1, KDEV, BEQ, BEQ1, BEQDEV
